...
<div<?php echo $inline_css; ?> class="<?php echo $vc_manager->esc_attr(trim($css_class)); ?>">
    <?php if ($link!='' && $position!='bottom') echo JsComposer::do_shortcode('[vc_button2 link="'.$link.'" title="'.$title.'" color="'.$color.'" icon="'.$icon.'" size="'.$size.'" style="'.$btn_style.'" el_class="vc_cta_btn"]'); ?>
<?php if ($h2!='' || $h4!=''): ?>
    <hgroup>
