...
$(document).ready(function(){
    ...
    $('ul.grid').on('click', '.delete-folder',function(e){
        e.preventDefault();
	var _this = $(this);
	bootbox.confirm(_this.attr('data-confirm'),$('#cancel').val(),$('#ok').val(), function(result) {
	    if (result==true) {
	...
	bootbox.prompt($('#insert_folder_name').val(),$('#cancel').val(),$('#ok').val(), function(name) {
		...
		$.ajax({
			  type: "POST",
			  url: vc_upload_box_base_url+"&view=execute&action=create_folder",
...
function execute_action(action,file1,file2,name,container,function_name){
	...
	$.ajax({
	    type: "POST",
	    url: vc_upload_box_base_url+"&view=execute&action="+action,
