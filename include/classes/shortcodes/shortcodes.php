	...
	abstract class WPBakeryVisualComposerAbstract {
		...
		public function addAction( $action, $method, $priority = 10 ) {
                        ...
                        if(method_exists($this, $method)){
                            ...
                            if(!isset(JsComposer::$VCBackofficeShortcodesAction[$action])){
                                JsComposer::$VCBackofficeShortcodesAction[$action] = array();
                            }
                            JsComposer::$VCBackofficeShortcodesAction[$action][] = array( &$this, $method );
                        }
...
//			return add_filter( $filter, array( &$this, $method ), $priority );
		...
		public function addShortCode( $tag, $func ) {                        
			JsComposer::add_shortcode( $tag, $func );//change this like wp shortcodes..
		}
	...
	abstract class WPBakeryShortCode extends WPBakeryVisualComposerAbstract {
		...
		public function printIconStyles() {
			...
			if(Tools::getvalue('controller') == 'Adminvccontentanywhere' || Tools::getvalue('controller') == 'Adminvcproducttabcreator'){
			 ...
			 echo "
             <style>
                 .vc_el-container #".$this->settings['base']." .vc_element-icon,
                 .wpb_".$this->settings['base'] ." .wpb_element_title .vc_element-icon {
                     background-image: url(" . $this->settings['icon']  . ");
                     background-image: url(" . $this->settings['icon']. ");
...
//			}
		...
		protected function registerCss( $param ) {
                        JsComposer::$registeredCSS[] = $param; // to load css in prestashop displayBackOfficeHeader hook
...
//			}
		...
		protected function loadTemplate( $atts, $content = null ) {
			...
			if ( $this->html_template ) {
				...
				trigger_error( sprintf( vc_manager()->l('Template file is missing for `%s` shortcode. Make sure you have `%s` file in your theme folder.'), $this->shortcode, 'wp-content/themes/your_theme/vc_templates/' . $this->shortcode . '.php' ) );
			}
			return $output;
		}
		...
		public function contentAdmin( $atts, $content = null ) {
			...
			if ( $content != NULL ) {
				$content = vc_manager()->wpautop( stripslashes( $content ) );
			}
...
//					$param_value = isset( $$param['param_name'] ) ? $$param['param_name'] : '';
		...
		public function isAdmin() {
                        ...
                        $return = isset(Context::getContext()->employee->id) && Tools::getValue('action') && preg_match( '/^wpb\_/', Tools::getValue('action') );                        
                        return $return;
...
//				wp_enqueue_script( 'waypoints' );
                            Context::getContext()->controller->addJS(vc_asset_url( 'lib/jquery-waypoints/waypoints.min.js' ));
...
---------------------------------------------------------- */
		...
		protected function outputTitle( $title ) {
            ...
            return  '<h4 class="wpb_element_title"><span class="vc_element-icon'.( !empty($icon) ? ' '.$icon : '' ).'"></span> '.vc_manager()->esc_attr($title).'</h4>';
		}
		...
		public function contentAdmin( $atts, $content = null ) {
			...
			if ( isset( $this->settings['params'] ) ) {
				...
				/*extract(JsComposer::shortcode_atts(
					$shortcode_attributes
					, $atts ) );*/
				$atts = JsComposer::shortcode_atts( $shortcode_attributes, $atts );
				extract($atts);
...
//			$param = apply_filters( 'vc_single_param_edit', $param );
			$param = Vc_Shortcode_Edit_Form::changeEditFormFieldParams($param) ;
		...
		protected function singleParamEditForm( $param, $param_value ) {
			$param_line = '';
			// Textfield - input                        
//                        if(isset($param['param_name']) && $param['param_name'] == 'content')
//                        {
//                            $langid = Context::getContext()->language->id;
//                            $param['param_name'] = $param['param_name']."_{$langid}";                            
//                        }
                        $vc_manager = vc_manager();
					...
					$param_line .= '<textarea id="wpb_tinymce_content" name="wpb_tinymce_content" class="wpb-textarea visual_composer_tinymce content textarea_html wp-editor-area rte autoload_rte">'.$param_value.'</textarea>';
                                        ...
                                        $(function(){
                                            if(typeof tinySetup == 'function'){
                                                tinySetup({
                                                    selector: "#wpb_tinymce_content",
                                                    setup: function(editor) {
                                                        editor.on('change', function(e) {
                                                            $('#wpb_tinymce_content').html(editor.getContent());
                                                        });
                                                    }
                                                });
                                            }else{
                                                var tmce = tinyMCE !== undefined ? tinyMCE : parent.tinyMCE;
                                                tmce.init({
                                                     selector: "#wpb_tinymce_content",
                                                     inline: false,
                                                     skin: "prestashop",
                                                     ...
                                                     verify_html : true,
                                                     extended_valid_elements : "em[class|name|id],br,br[*],span[*],i[*],script[src|type|language]",
                                                     setup: function(editor) {
                                                         editor.on('change', function(e) {
                                                             $('#wpb_tinymce_content').html(editor.getContent());
                                                         });
                                        ...
                                        $param_line .= '</div>';
//                                        $param_line .= '<input type="hidden" name="' . $param['param_name'] . '" class="vc_textarea_html_content wpb_vc_param_value content" value="' . $param_value . '" />';
                                        $param_line .= '</div>';
                                        
//					$param_line .= '<div class="updated"><p>' . $vc_manager->l( 'Field type is changed from `textarea_html` to `textarea`, because it is already used by `' . $this->html_editor_already_is_used . '` field. Textarea_html field\'s type can be used only once per shortcode.' ) . '</p></div>';
				} else {
				...
				//$param_line .= '<a class="button gallery_widget_add_images" href="#" title="'.$vc_manager->l('Add images').'">'.$vc_manager->l('Add images').'</a>';
				...
				$param_line .= '</div>';
				$param_line .= '<a class="gallery_widget_add_images" href="#" title="' . $vc_manager->l('Add images') . '">' . $vc_manager->l('Add images') . '</a>'; //class: button
				...
				$param_line .= '<input type="hidden" class="wpb_vc_param_value gallery_widget_attached_images_ids ' . $param['param_name'] . ' ' . $param['type'] . '" name="' . $param['param_name'] . '" value="' . $param_value . '"/>';
				//$param_line .= '<a class="button gallery_widget_add_images" href="#" use-single="true" title="'.$vc_manager->l('Add image').'">'.$vc_manager->l('Add image').'</a>';
				$param_line .= '<div class="gallery_widget_attached_images">';
				$param_line .= '<ul class="gallery_widget_attached_images_list">';
				$param_line .= ( $param_value != '' ) ? fieldAttachedImages( explode( ",", $param_value ) ) : '';
				$param_line .= '</ul>';
				...
				// $param_line .= siteAttachedImages(explode(",", $param_value));
				$param_line .= '</div>';
				$param_line .= '<a class="gallery_widget_add_images" href="#" use-single="true" title="' . $vc_manager->l('Add image') . '">' . $vc_manager->l('Add image') . '</a>'; //class: button
				//$param_line .= '<div class="wpb_clear"></div>';
			} //
		...
		protected function getTinyHtmlTextArea( $param = array(), $param_value ) {
			...
			//$upload_media_btns = '<div class="wpb_media-buttons hide-if-no-js"> '.vc_manager()->l('Upload/Insert').' <a title="'.vc_manager()->l('Add an Image').'" class="wpb_insert-image" href="#"><img alt="'.vc_manager()->l('Add an Image').'" src="'.home_url().'/wp-admin/images/media-button-image.gif"></a> <a class="wpb_switch-editors" title="'.vc_manager()->l('Switch Editors').'" href="#">HTML mode</a></div>';
...
if ( ! class_exists( 'WPBakeryShortCodesContainer' ) ) {
	abstract class WPBakeryShortCodesContainer extends WPBakeryShortCode {
		...
		public function getColumnControls( $controls, $extended_css = '' ) {
			...
			$controls_end = '</div>';
                        $vc_manager = vc_manager();
			if ( $extended_css == 'bottom-controls' ) $control_title = sprintf( $vc_manager->l('Append to this %s'), strtolower( $this->settings( 'name' ) ) );
			else $control_title = sprintf( $vc_manager->l('Prepend to this %s'), strtolower( $this->settings( 'name' ) ) );

			$controls_add = ' <a class="vc_control column_add" href="#" title="' . $control_title . '"><span class="vc_icon"></span></a>';
			$controls_edit = ' <a class="vc_control column_edit" href="#" title="' . sprintf( $vc_manager->l('Edit this %s'), strtolower( $this->settings( 'name' ) ) ) . '"><span class="vc_icon"></span></a>';
			$controls_clone = '<a class="vc_control column_clone" href="#" title="' . sprintf( $vc_manager->l('Clone this %s'), strtolower( $this->settings( 'name' ) ) ) . '"><span class="vc_icon"></span></a>';

			$controls_delete = '<a class="vc_control column_delete" href="#" title="' . sprintf( $vc_manager->l('Delete this %s'), strtolower( $this->settings( 'name' ) ) ) . '"><span class="vc_icon"></span></a>';
		...
		public function contentAdmin( $atts, $content = null ) {
			...
			$atts = JsComposer::shortcode_atts( $this->predefined_atts, $atts );
			extract( $atts );
			$output = '';
                        if(is_string($width)){
                            $width = array($width);
			...
			for ( $i = 0; $i < count( $width ); $i ++ ) {
				...
				$output .= JsComposer::do_shortcode( JsComposer::shortcode_unautop( $content ) );
				$output .= '</div>';
				if ( isset( $this->settings['params'] ) ) {
					$inner = '';
					foreach ( $this->settings['params'] as $param ) {
		...
		protected function outputTitle( $title ) {
			$icon = $this->settings('icon');
			if( filter_var( $icon, FILTER_VALIDATE_URL ) ) $icon = '';
			return  '<h4 class="wpb_element_title"><span title="'.$title.'" class="vc_element-icon'.( !empty($icon) ? ' '.$icon : '' ).'"></span></h4>';
		}
	}
}
	...
	class WPBakeryShortCode_Settings extends WPBakeryShortCode_UniversalAdmin {
		...
		public function contentAdmin( $atts, $content = null ) {
			...
			if ( isset( $this->settings['params'] ) ) {
                ...
                $attrs = JsComposer::admin_shortcode_atts(
					$shortcode_attributes
					, $atts );
				extract( $attrs );
                
//				$editor_css_classes = apply_filters( 'vc_edit_form_class', array( 'wpb_edit_form_elements' ) );
				$editor_css_classes = array( 'vc_row wpb_edit_form_elements' );
