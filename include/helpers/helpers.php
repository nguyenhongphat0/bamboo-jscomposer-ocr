...
function wpb_getImageBySize( $params = array( 'post_id' => NULL, 'attach_id' => NULL, 'thumb_size' => '', 'class' => '' ) ) {
    
    if(isset($params['attach_id']) && !empty($params['attach_id'])){
        
        $thumbnail = JsComposer::get_media_thumbnail_url($params['attach_id']);
        
        if(empty($thumbnail)) 
            return array();
        
        $thumb_class = ( isset( $params['class'] ) && $params['class'] != '' ) ? $params['class'] . ' ' : '';
        $mainImg = $bigImg = JsComposer::$_url.'uploads/'.$thumbnail;
        
        if(!empty($params['thumb_size'])){
            $type = VcImageType::getImageTypeByName($params['thumb_size']);
            $imgdir = vc_manager()->composer_settings['UPLOADS_DIR'];

            if(!empty($type)){
                $filerealname = Tools::substr($thumbnail, 0, Tools::strrpos($thumbnail, '.'));
                $ext = substr($thumbnail, Tools::strrpos($thumbnail, '.'));

                foreach($type as $imageType){
                    $newfile = $filerealname.'-'.$imageType['name'].$ext;
                    if(file_exists($imgdir.$newfile)){
                        $mainImg = JsComposer::$_url.'uploads/'.$newfile;
                    }
                }
        ...
        $alt =JsComposer::get_media_alt($params['attach_id']);
        $thumbnail = '<img class="' . $thumb_class . '"  alt="'.$alt .'" src="'.JsComposer::ModifyImageUrl($mainImg). '" />';

        return array( 'thumbnail' => $thumbnail, 'p_img_large' => $bigImg );
    }
...
function wpb_js_remove_wpautop( $content, $autop = false ) {
	...
	if ( $autop ) { // Possible to use !preg_match('('.WPBMap::getTagsRegexp().')', $content)
		$content = JsComposer::wpautop( preg_replace( '/<\/?p\>/', "\n", $content ) . "\n" );
	}
	return JsComposer::do_shortcode( JsComposer::shortcode_unautop( $content) );
}
...
//		$thumb_src = wp_get_attachment_image_src( $th_id, 'thumbnail' );
		...
		if ( isset($result['file_name']) && !empty($result['file_name']) ) {
			...
			$output .= '
			<li class="added">
				<img rel="' . $th_id . '" src="' . JsComposer::ModifyImageUrl($thumb_src) . '" />
...
function vc_convert_shortcode( $m ) {
	...
	$shortcode_attr = JsComposer::shortcode_parse_atts( $attr_string );
...
function vc_convert_tab_inner_shortcode( $m ) {
	...
	extract( JsComposer::shortcode_atts( array(
		...
		'el_class' => '',
		'el_position' => ''
	), JsComposer::shortcode_parse_atts( $attr_string ) ) );
	$pattern = vc_manager()->get_shortcode_regex();
	$result .= "[{$m_one}{$tag} {$attr_string}]" . preg_replace_callback( "/{$pattern}/s", 'vc_convert_inner_shortcode', $content ) . "[/{$tag}{$m_four}]";
	return $result;
}
...
function vc_convert_inner_shortcode( $m ) {
	...
	if ( $width != '1/1' ) {
		...
		$attr = '';
		foreach ( JsComposer::shortcode_parse_atts( $attr_string ) as $key => $value ) {
