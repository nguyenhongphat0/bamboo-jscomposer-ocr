...
//		$this->post_url = get_permalink( $this->post_id );
		...
		if ( ! $this->inlineEnabled()) header( 'Location: ' . $this->post_url );
...
//                include vc_manager()->composer_settings['PARAMS_DIR'].'/options/options.php';
...
//		return apply_filters( 'vc_inline_template_content', $this->template_content );
	...
	function parseShortcodesString( $content, $is_container = false, $parent_id = false ) {
		...
		foreach ( $found[2] as $index => $s ) {
			...
			$shortcode = array( 'tag' => $s, 'attrs_query' => $found[3][$index], 'attrs' => JsComposer::shortcode_parse_atts( $found[3][$index] ), 'id' => $id, 'parent_id' => $parent_id );
