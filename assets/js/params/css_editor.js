...
(function ($) {
  var preloader_url = vc_ajaxurl.replace(/admin\-ajax\.php/, 'images/wpspin_light.gif'),
      template_options = {
        evaluate:    /<#([\s\S]+?)#>/g,
        interpolate: /\{\{\{([\s\S]+?)\}\}\}/g,
        escape:      /\{\{([^\}]+?)\}\}(?!\})/g
...
//        console.log(this._css_editor.$el);
...
//                fieldElm.value = url; 
//                var selection = elem.get('selection');
//                console.log(selection);
                
                elem._css_editor.$el.find('.vc_background-image .vc_image').html('<li class="added">  <div class="inner" style="width: 75px; height: 75px; overflow: hidden;text-align: center;">    <img src="'+url+'" data-image-id="412" class="vc_ce-image">  </div>  <a href="#" class="icon-remove"></a></li>');
             }
         });
...
//          console.log(name);
          ...
          } else if(name.match(/border\-[\w]+\-style/)){
              this.$el.find('[name=border_style]').val(value);
          } else if(name.match(/border\-[\w]+\-color/)){
...
//          console.info('here....'); // work here..
        this.attrs.border = this.attrs['border-width'] + ' ' + style + ' ' + color;
        this.attrs['border-width'] = undefined;
      } else {
//          e.g. border-width: 2px 1px 3px 1px make issue for border-style and border-color properties
...
//                this.attrs['border-' + side + '-width'] = '0px';
//            }
//        }, this);
      }
      if(color.match(/^rgba/)) {
