...
!function($) {
    ...
    window.init_textarea_html_wp_temp = function($element) {
    ...
    Suggester.prototype = {
        ...
        init: function() {
            _.bindAll(this, 'registerRemoveAction', 'addItem', 'buildSource', 'itemSelected', 'labelClick', 'setFocus', 'resize');
            var that = this;
            window.autoSuggestProto = that;
//            $(document.body).off('click', '.autosuggest-list .delCategory');
//            $(document.body).on('click', '.autosuggest-list .delCategory', this.removeItem);
            this.registerRemoveAction();
            ...
            this.$el.autocomplete(window.vc_ajaxurl+'&action=wpb_get_loop_suggestion&field='+this.suggester, {
                minChars: 1,
                autoFill: true,
                max: 20,
                matchContains: true,
            ...
            this.$el.setOptions({
                extraParams: {
                    excludeIds: this.getExistingIds(this.$el)
                }
            });
...
//            }
        ...
        registerRemoveAction: function(){
            $(document.body).off('click', '.autosuggest-list .delCategory');
            $(document.body).on('click', '.autosuggest-list .delCategory', this.removeItem);
        },
        removeItem: function(event){
            event.preventDefault();
            var elemindex = $(this).closest('.form-control-static').index();
            if(elemindex != 'undefined' && elemindex != -1){
                var mainInput = $(this).closest('.vc_suggest-field').find('input[data-suggest-value]');
                var text = mainInput.val().split(',');
                if(typeof text[elemindex] != 'undefined'){
                    text.splice(elemindex, 1);
                    mainInput.val(text.join(','));
                    mainInput.trigger('change');
                    $(this).closest('.form-control-static').remove();
                    mainInput.closest('.vc_suggest-field').find('.ac_input').setOptions({
                        extraParams: {
                            excludeIds: mainInput.val()
                        }
                    });
                }
            }
        },
        addItem: function(event, data, formatted)
        {
            if (data == null)
                return false;
            ...
            var elem = this.$el;
            var parent = elem.closest('.vc_suggest-field');
            var fieldval = parent.find('input[data-suggest-value]').val();
            if(fieldval != ''){
                fieldval += ',';
            }
            fieldval += productId;
            parent.find('input[data-suggest-value]').val(fieldval);
            parent.find('input[data-suggest-value]').trigger('change');
            parent.find('.autosuggest-list').append('<div class="form-control-static"><button type="button" class="delCategory btn btn-default" name="' + productId + '"><i class="icon-remove text-danger"></i></button>&nbsp;' + productName + '</div>');
            
            elem.val(''); // clear the autocomplete field.
            this.registerRemoveAction();
            elem.setOptions({
                extraParams: {
                    excludeIds: this.getExistingIds(elem)
                }
            });

        },
        getExistingIds: function(elem){
            return elem.closest('.vc_suggest-field').find('input[data-suggest-value]').val();
        },
        resize: function() {
...
//            console.log(this.loop_editor_view);
...
//        }
      ...
      var checkVal = function(opts){
          
          if($input.data('json') !== undefined && $input.data('json') != ''){
              var values = $input.data('json');
            switch(opts.type){
                case 'filepicker':
                    return values.url;
                    break;
                case 'listbox':
                    return values.target === opts.value;
                    break;
                case 'textbox':
                    return values.title;
                    break;
      ...
      var e = parent.tinymce.activeEditor;
        e.windowManager.open({
            ...
            body: [{
                ...
                onchange: '',
                onkeyup: '',
                value : checkVal({name:"url",type:"filepicker"})
            },{
                name: "title",
                type: "textbox",
                size: 40,
                ...
                onchange: '',
                value : checkVal({name:"title",type:"textbox"})
            },{
                name: "target",
                type: "listbox",
                label: "Target",
                values: [{
                    text: "None",
                    value: "_self",
                    selected : checkVal({name:"target",type:"listbox",value:"_self"})
                },{
                    text: "New window",
                    value: "_blank",
                    selected : checkVal({name:"target",type:"listbox",value:"_blank"})
                }]
            }],
            onSubmit: function(t) {
                var data = t.data;
                var options = { url : data.url, title : data.title, target : data.target };
                
                var string = _.map(options, function(value, key){
                if(_.isString(value) && value.length > 0) {
                    return key + ':' + encodeURIComponent(value);
                }
                }).join('|');
                $input.val(string);
                $block.children('span.title-label').html(options.title);
                $block.children('span.url-label').html(options.url);
    ...
    VcSortedList.prototype = {
        ...
        init: function() {
            ...
            for(var i in selected_data) {
                ...
                var $control = control_settings.length ? this.$toolbar.find('[data-element=' + decodeURIComponent(control_settings[0]) + ']') : false;
    ...
    var GoogleFonts = Backbone.View.extend({
        ...
        fontStyleDropdownRender: function() {
            ...
            for( var str_inner in str_arr ) {
                ...
                str_inner = parseInt(str_inner);
                if(!_.isNumber(str_inner) || isNaN(str_inner)) continue;
