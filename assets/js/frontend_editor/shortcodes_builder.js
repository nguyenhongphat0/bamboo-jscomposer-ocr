...
(function($) {
  ...
  vc.ShortcodesBuilder.prototype = {
    ...
    loadImageGalleryJs: function(jstype) {
      vc.frame_window.vc_iframe.loadImageGalleryJs(jstype);
      return true;
    },
    loadCarousel: function() {
      vc.frame_window.vc_iframe.loadCarousel();
      return true;
    },
    ...
    modelsToString: function(models) {
      ...
      _.each(models, function(model) {
        ...
        string += wp.shortcode.string({
          ...
          content: content,
          type: content == '' && !is_container ? 'single' : ''
        });
    ...
    save: function(status) {
      var string = this.getContent(),
        data = {
          ...
          val_identifier: $('#val_identifier').val(),
      ...
      this.ajax(data, window.vc_ajaxurl)
        .done(function(){
          vc.unsetDataChanged();
