	...
	class WPBakeryVisualComposerCssEditor {
		...
		function render() {
			...
			$output .= '<div class="vc_col-xs-5 vc_settings">'
			  ...
			  . '</div>';
			$output .= '<input name="' . $this->setting( 'param_name' ) . '" class="wpb_vc_param_value  ' . $this->setting( 'param_name' ) . ' ' . $this->setting( 'type' ) . '_field" type="hidden" value="' . $vc_manager->esc_attr( $this->value() ) . '"/>';
			...
			if(!$this->js_script_appended) {
				...
				$output .= "\n\n" . '<doscript>JS::' . vc_asset_url( 'js/params/css_editor.js' ) . '</doscript>';
...
//			return apply_filters( 'vc_css_editor', $output );
		...
		// 	  . '</ul>'
		// 	  . '<a href="#" class="vc_add-image">' . vc_manager()->l('Add image') . '</a>';
		// }
        function getBackgroundImageControl() {
            return '<ul class="vc_image">'
              . '</ul>'
