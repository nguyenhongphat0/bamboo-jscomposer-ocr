...
if ( ! class_exists( 'Vc_Automap_Model' ) ) {
	...
	class Vc_Automap_Model {
		...
		final protected static function loadOptionData() {
			if ( is_null( self::$option_data ) ) self::$option_data = Configuration::get( self::$option_name );
...
//			add_action( 'wp_ajax_vc_automapper', array( &$this, 'goAction' ) );
		...
		public function build() {
			...
			wp_localize_script( 'wpb_js_composer_automapper', 'i18nLocaleVcAutomapper', array(
				'are_you_sure_delete' => $jscomposer->l('Are you sure you want to delete this shortcode?'),
				'are_you_sure_delete_param' => __( "Are you sure you want to delete the shortcode's param?", 'js_composer' ),
				'my_shortcodes_category' => $jscomposer->l('My shortcodes'),
				...
				'new_shortcode_mapped' => $jscomposer->l('New shortcode mapped from string!'),
				'shortcode_updated' => $jscomposer->l('Shortcode updated!'),
				'error_content_param_not_manually' => $jscomposer->l('Content param can not be added manually, please use checkbox.'),
				'error_param_already_exists' => $jscomposer->l('Param %s already exists. Param names must be unique.'),
				'error_wrong_param_name' => $jscomposer->l('Please use only letters, numbers and underscore for param name'),
				'error_enter_valid_shortcode' => $jscomposer->l('Please enter valid shortcode to parse!')
			) );
		}
		...
		<script type="text/html" id="vc_automapper-form-tpl">
			...
			<div class="vc_wrapper">
				...
				<div class="vc_field vc_is-container">
					<label for="vc_atm-is-container"><input type="checkbox" name="is_container" id="vc_atm-is-container"
		...
		public function goAction() {
                        $jscomposer = JsComposer::getInstance();
			$action = $jscomposer->vc_post_param( 'vc_action' );                        
