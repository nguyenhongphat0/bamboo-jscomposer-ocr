...
//                $output .= ob_get_clean();
		...
		if ( isset( $param['holder'] ) == false || $param['holder'] == 'hidden' ) {
			...
			if ( ( $param['type'] ) == 'attach_image' ) {
                ...
                $img = wpb_getImageBySize( array( 'attach_id' => (int)preg_replace( '/[^\d]/', '', $value ), 'thumb_size' => $thumb_size ) );

				$this->setSettings('logo', ( $img ? $img['thumbnail'] : '<img width="150" height="150" src="' . vc_asset_url( 'vc/blank.gif' ) . '" class="attachment-thumbnail vc_element-icon"  data-name="' . $param_name . '" alt="" title="" style="display: none;" />' ) . '<span class="no_image_image vc_element-icon' . ( !empty($element_icon) ? ' '.$element_icon : '' ) . ( $img && ! empty( $img['p_img_large'][0] ) ? ' image-exists' : '' ) . '" /><a href="#" class="column_edit_trigger' . ( $img && ! empty( $img['p_img_large'][0] ) ? ' image-exists' : '' ) . '">' . vc_manager()->l( 'Add image') . '</a>');
	...
	protected function outputTitleTrue($title) {
		return  '<h4 class="wpb_element_title">'.vc_manager()->l($title).' '.$this->settings('logo').'</h4>';
	}
}