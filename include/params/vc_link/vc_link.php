...
function vc_vc_link_form_field( $settings, $value ) {
	...
	return '<div class="vc_link">'
	  ...
	  . '<a href="#" class="button vc_link-build ' . $settings['param_name'] . '_button">' . $vc_manager->l('Select URL') . '</a> <span class="vc_link_label_title vc_link_label">' . $vc_manager->l('Title') . ':</span> <span class="title-label">' . $link['title'] . '</span> <span class="vc_link_label">' . $vc_manager->l('URL') . ':</span> <span class="url-label">' . $link['url'] . ' ' . $link['target'] . '</span>'
	  . '</div>';
}
