...
//            'click form.defaultForm button[name^="submit"]':'saveAdditionalVal',
//            'submit form.defaultForm':'saveAdditionalVal',
...
//            this.$el = $(this.$el);
            ...
            $('form.defaultForm').on('submit',this.saveAdditionalVal);
        ...
        saveAdditionalVal: function(){
            
            if(window.ace && window.ace.edit){
                var editor = window.ace.edit('wpb_csseditor');
                var cssval = editor.getSession().getValue(); 
                if(window.page_id !== null)
                    $.ajax({
                        type:'POST',
                        url:window.vc_ajaxurl,
                        data:{
                          action :'wpb_save_css_values',                  
                          post_id : window.page_id,
                          type : window.page_type,
                          id_lang: window.sds_id_lang(),
                          css: cssval
                        }
...
//            $('form.defaultForm').submit(); // don't forget to uncomment it. its required..
        ...
        render:function () {
            ...
            if (this.accessPolicy !== 'only') {                
                ...
                if(vc_backend_enabled) back = '<a class="wpb_switch-to-composer" href="#">' + window.i18nLocale.main_button_title_backend_editor + '</a>'
                if(vc_frontend_enabled || vc_backend_enabled) insert_after = '<div class="composer-switch"><span class="logo-icon"></span><span class="vc_spacer"></span>' + back + front + '</div>';

                this.$buttonsContainer = $(insert_after).insertAfter('div#titlediv');
...
//            this.$post = 'div.mce-tinymce';
            this.$post = $('[name='+window.vcActiveTextFieldName+'_'+window.sds_id_lang()+']');
            
            if(this.$post.closest('.form-group').hasClass('translatable-field')){ // to handle multilangual text boxes
                this.$post = this.$post.closest('.form-group').parents('.form-group');
            }else{
                this.$post = this.$post.closest('.form-group');
            }
        ...
        setSortable:function () {
            ...
            $('.wpb_column_container').sortable({
                ...
                over:function (event, ui) {
                    ...
                    if(ui.sender != null && ui.sender.length && !ui.sender.find('[data-element_type]:visible').length) {
...
//            $('#publish').click();
            ...
            $('form.form-horizontal .panel-footer button[name^="submit"]').trigger('click');
        },
        preview:function() {
            if(window.page_id !== null)
                $.ajax({
