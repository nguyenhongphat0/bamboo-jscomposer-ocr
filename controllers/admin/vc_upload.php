...
class VC_uploadController extends ModuleAdminController 
{
    ...
    public function initProcess()
    {
        ...
        $file = _PS_ROOT_DIR_.'/modules/jscomposer/views/'.$view.'.php';
        if(Tools::getValue('action') && Tools::getValue('action') == 'remove'){
    ...
    public function delete_uploaded_file(){
        ...
        $filename = Tools::getValue('img');

        $imgdir = _PS_ROOT_DIR_.'/modules/jscomposer/uploads/';

        if(!empty($filename) && file_exists("{$imgdir}{$filename}")){

            Tools::deleteFile("{$imgdir}{$filename}");
            ...
            echo Tools::jsonEncode(array(
                ...
                'output'=>  JsComposer::get_uploaded_files_markup(JsComposer::get_uploaded_files_result())
    ...
    public function initHeader()
    {
        ...
        foreach ($quick_access as $index => $quick)
        {
            ...
            else
            {
                ...
                {
                    if (Tools::strpos($admin_tab[1], '&'))
                        $admin_tab[1] = Tools::substr($admin_tab[1], 0, Tools::strpos($admin_tab[1], '&'));
