...
---------------------------------------------------------- */
	public function getColumnControls( $controls, $extended_css = '' ) {
		...
		$controls_layout .= '<br/><a class="vc_control-set-column set_columns custom_columns" data-cells="custom" data-cells-mask="custom" title="' . $vc_manager->l('Custom layout') . '">' . $vc_manager->l('Custom') . '</a> ';
	...
	public function contentAdmin( $atts, $content = null ) {
		...
		for ( $i = 0; $i < count( $width ); $i ++ ) {
			...
			if ( $content == '' && ! empty( $this->settings["default_content_in_template"] ) ) {
				$output .= JsComposer::do_shortcode( JsComposer::shortcode_unautop( $this->settings["default_content_in_template"] ) );
			} else {
