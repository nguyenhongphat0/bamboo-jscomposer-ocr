...
}elseif(isset($vcparams['fc']) && $vcparams['fc'] == 'module'){
	$module = Validate::isModuleName(Tools::getValue('module')) ? Tools::getValue('module') : '';
	if(!empty($module))
	{
		unset($vcparams['fc'], $vcparams['module']);
		$page_link = $linkobj->getModuleLink($module, $page_type, $vcparams, null, (int)$id_lang);
	}else{
		$page_link = Context::getContext()->shop->getBaseURL();
	}
}else{
