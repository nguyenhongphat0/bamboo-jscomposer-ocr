	...
	switch ($_GET['action'])
	{
		...
		case 'delete_folder':
			if ($delete_folders)
			{
                ...
                $subdirectory = str_replace(vc_manager()->composer_settings['UPLOADS_DIR'], '', $path_thumb);
                if(strpos($path_thumb, vc_manager()->composer_settings['UPLOADS_DIR']) === FALSE){
                    $path = $path_thumb = vc_manager()->composer_settings['UPLOADS_DIR'].$path_thumb;
                }
                if(!empty($subdirectory)){
                    $db = Db::getInstance();
                    $sql = "DELETE FROM "._DB_PREFIX_.JsComposer::$VC_MEDIA." WHERE subdir LIKE '{$subdirectory}/%'";
                    $db->execute($sql);
                }
...
//                        $sql = "DELETE FROM "._DB_PREFIX_.JsComposer::$VC_MEDIA." WHERE subdir LIKE '{$subdirectory}/%'";
//                        $db->execute($sql);
                        $sql = "SELECT * FROM "._DB_PREFIX_.JsComposer::$VC_MEDIA." WHERE subdir LIKE '{$subdirectory}/%'";
